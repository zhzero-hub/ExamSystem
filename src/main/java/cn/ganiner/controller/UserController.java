...
public class UserController {
    ...
    @ResponseBody
    public String NewPass(User user, Model model, HttpServletResponse response) {
        userService.updateuser(user);
        model.addAttribute("message", "密码更新成功");
        try {
            response.getWriter().write("更新信息成功");
        }
        catch(IOException e) {
            e.printStackTrace();
        }
    ...
    public String createchoice(Choice choice, Model model){
        try{
            ...
            System.out.println("新建选择题: " + choice.toString());
            //model.addAttribute("message", "保存章节成功");
        }catch (Exception e) {
            e.printStackTrace();
        }
        return "redirect:/user/findallchoice.do";
    }
